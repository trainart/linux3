...
```bash
host -t a gmail.com 8.8.8.8
```

```bash
dig -t ns ya.ru @8.8.8.8
```
...
By listening port (TCP+UDP)

```bash
netstat -4nlptu | grep named
```

By listening port (only UDP)

```bash
ss -4nlpu | grep named
```

Notice that BIND listens only to `127.0.0.1`. We will change that below.

### Turn off Firewalld & SELinux 

Firewalld & SELinux (Security-Enhanced Linux) may cause problems, 
so we will turn them off for this training.
...
Turn off SELinux if needed (this will make effect only until reboot)
...
If you see `SELINUX=disabled`, then nothing should be done, 
if not you need to edit that file, so that SELinux will be disabled after reboot.
...
First stop the DNS server process
...
> REMEMBER AGAIN
> When BIND is stoppped we work with configuration in `etc/named.conf`
> When it is started we work with chrooted version in `/var/named/chroot/etc/named.conf`


Now in section `options { … }` of `etc/named.conf` <br>
we need to make 2 changes 

* "_listen-on port 53 { **127.0.0.1;** };_" -> "_listen-on port 53 { **any;** };_" 
* "_allow-query     { **localhost;** };_" -> "_allow-query     { **any;** };_" 

Instead of manually doing that, it can be done with below commands


Show what we need to change

```bash
grep -n 127.0.0.1 /etc/named.conf
```

Make changes

```bash
sed -i.backup 's/127.0.0.1/any/' /etc/named.conf
```

Check what changed

```bash
diff /etc/named.conf.backup /etc/named.conf
```

Show what we need to change

```bash
grep -n localhost /etc/named.conf
```

Make changes

```bash
sed -i.backup 's/localhost/any/' /etc/named.conf
```
...
Start BIND with new configuration

```bash
systemctl start named-chroot 
```

Check that BIND now listens to all IPs
...
> **lt00** (note it is in many places pattern and filename) and  
...
Protect from changes (set **Immutable** attribure)
