...
Enable Logging
```bash
echo "log measurements statistics tracking" >>/etc/chrony.conf
```

Use local clock as last resort reference (stratum 10) if no upstream connection.
...
To enable Chrony act as Server (to serve for remote hosts) 
enable following line to `/etc/chrony.conf`
...
and Students can configure their Chrony to use that NTP Server
