...
```bash
sudo -u student bash -c 'echo "Student's file" > /opt/acltest/student-file'
```
...
* As `acluser` create empty file `/srv/tryacl/MYFILE` 
* As `acluser` add the text `testing ACL` in the file `/srv/tryacl/MYFILE` 
* Now remove all ACL rules for directory `/srv/tryacl`
* Show that `acluser` no longer has special permissions for directory `/srv/tryacl` 
