...
```bash
sudo -u student mkdir /opt/acltest/student-dir
```
...
```bash
sudo -u student touch /opt/acltest/student-file
```
...
As you may understand with ACL rules we provided separate additional permission to user `student` for directory `/opt/acltest/`
...
```bash
sudo -u student touch /opt/acltest/new-file
```
...
```bash
sudo -u student echo "Hello"  > /opt/acltest/student-file
```

```bash
sudo -u student echo "Hello again"  >> /opt/acltest/student-file
```
...
* Show that `acluser` don't have any special permissions to create new file/dir in `/srv/tryacl` directory
* Show that `acluser` still can modify file `/srv/tryacl/MYFILE` 
* As `acluser` add text "ADDING NEW TEXT" to the file `/srv/tryacl/MYFILE`
